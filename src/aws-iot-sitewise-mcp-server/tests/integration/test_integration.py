...
from awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_asset_models import create_asset_model, describe_asset_model
...
from awslabs.aws_iot_sitewise_mcp_server.tools.sitewise_gateways import create_gateway, describe_gateway
...
class TestAssetModelLifecycle:
    ...
    def test_asset_model_create_and_cleanup(self, sitewise_tracker):
        ...
        if result["success"]:
            ...
            assert wait_for_asset_model_active(
                client, model_id
            ...
            assert any(
                err in result.get("error_code", "") for err in expected_errors
        ...
        with sitewise_test_resources() as tracker:
            ...
            if asset_result["success"]:
                ...
                assert "AccessDenied" in asset_result.get(
                    "error_code", ""
                ) or "Unauthorized" in asset_result.get(
                    "error_code", ""
    ...
    def test_asset_hierarchy_with_cleanup(self):
        ...
        with sitewise_test_resources() as tracker:
            ...
            parent_model_result = create_asset_model(
                ...
                asset_model_hierarchies=[{"name": "Children", "childAssetModelId": child_model_id}],
            )
        ...
        with sitewise_test_resources() as tracker:
            ...
            if portal_result["success"]:
                ...
                assert "AccessDenied" in portal_result.get(
                    "error_code", ""
                ) or "Unauthorized" in portal_result.get(
                    "error_code", ""
